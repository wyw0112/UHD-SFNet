...
im_local = Image.open("demo/02_indoor_hazy.jpg")
im_trg_list = [Image.open("demo/02_outdoor_hazy.jpg"),
               Image.open("demo/03_indoor_hazy.jpg")]
...
for client_idx, im_trg in enumerate(im_trg_list):
    ...
    draw_image(np.clip((np.log(amp_target) / np.max(np.log(amp_target))).transpose((1, 2, 0)), 0, 1))
